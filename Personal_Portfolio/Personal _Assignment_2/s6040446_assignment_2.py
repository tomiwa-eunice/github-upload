...
plt.subplot(1, 1, 1)
# list comprehension
plt.barh(energy_dict['countries'], energyDifference, color=['b' if i >=0 else 'r' for i in energyDifference])
plt.title('Energy Consumption Difference \n between 2000 and 2016')
plt.xlabel('Oil Equivalent (kg)')
red_patch = patches.Patch(color='r', label='-ve Energy Diff')   # sets labels for the different colors
blue_patch = patches.Patch(color='b', label='+ve Energy Diff')
